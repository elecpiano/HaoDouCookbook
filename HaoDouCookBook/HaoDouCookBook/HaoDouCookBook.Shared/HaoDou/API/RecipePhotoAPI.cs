...
namespace HaoDouCookBook.HaoDou.API
{
    public class RecipePhotoAPI
    {
        ...
        public static async Task GetProdcuts(int offset, int limit, int type, int sort, string sign, int? uid, string uuid, Action<ProductPageData> onSuccess, Action<Error> onFail)
        {
            ...
            postRequest.AddPostData("sort", sort.ToString());
            ...
            string cacheFileName = string.Format("{0}-{1}-{2}-{3}", methodName, sort.ToString(), type.ToString(), offset.ToString(), limit.ToString());
